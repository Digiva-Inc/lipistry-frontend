...
export default function WarehousePanel() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    switch (activeTab) {
      ...
      case "returns":
        return order.status === "return_approved" || order.status === "returned";
      case "completed":
  ...
  const returnsCount = orders.filter(o => o.status === "return_approved").length;
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {order.status === "submitted_warehouse" && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold rounded-lg shadow-sm transition-all cursor-pointer"
                          >
                            Confirm Order
                          </button>
                        )}
                        {order.status === "confirmed" && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-indigo-650 hover:bg-indigo-700 text-white text-[11px] font-bold rounded-lg shadow-sm transition-all cursor-pointer"
                          >
                            Ship Order
                          </button>
                        )}
                        {order.status === "shipped" && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-[11px] font-bold rounded-lg shadow-sm transition-all cursor-pointer"
                          >
                            Out for Delivery
                          </button>
                        )}
                        {order.status === "out_for_delivery" && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-emerald-605 bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold rounded-lg shadow-sm transition-all cursor-pointer"
                          >
                            Mark Delivered
                          </button>
                        )}
                        {order.status === "return_approved" && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-teal-650 hover:bg-teal-700 text-white text-[11px] font-bold rounded-lg shadow-sm transition-all cursor-pointer"
                          >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-slate-700 hover:bg-slate-100 border border-slate-200 font-bold transition-all cursor-pointer shadow-sm text-[11px]"
                        >
                          <span>Pack List</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="glass-card rounded-2xl border border-slate-200 p-6 w-full max-w-xl relative overflow-y-auto max-h-[90vh] shadow-2xl flex flex-col text-left">
            ...
            <div className="mb-4 flex items-center gap-2 shrink-0 border-b border-slate-100 pb-3">
              ...
              <div>
                <h2 className="text-base font-extrabold text-slate-900">Fulfillment packing list</h2>
            ...
            {modalLoading ? (
              ...
              <div className="space-y-4 overflow-y-auto pr-1 flex-1">
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs">
                  <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block mb-1">Shipping Destination</span>
